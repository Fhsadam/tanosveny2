...
CREATE TABLE np (id INT PRIMARY KEY, nev VARCHAR(120) NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_hungarian_ci;
CREATE TABLE telepules (id INT PRIMARY KEY, nev VARCHAR(80) NOT NULL, npid INT NOT NULL, INDEX(npid), CONSTRAINT fk_telepules_np FOREIGN KEY (npid) REFERENCES np(id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_hungarian_ci;
CREATE TABLE ut (azon INT AUTO_INCREMENT PRIMARY KEY, nev VARCHAR(160) NOT NULL, hossz DECIMAL(5,2) NOT NULL, allomas INT NOT NULL, ido DECIMAL(5,2) NOT NULL, vezetes TINYINT(1) NOT NULL DEFAULT 0, telepulesid INT NOT NULL, INDEX(telepulesid), CONSTRAINT fk_ut_telepules FOREIGN KEY (telepulesid) REFERENCES telepules(id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_hungarian_ci;
CREATE TABLE users (id INT AUTO_INCREMENT PRIMARY KEY, csaladi_nev VARCHAR(80) NOT NULL, utonev VARCHAR(80) NOT NULL, login VARCHAR(80) NOT NULL UNIQUE, password_hash VARCHAR(255) NOT NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_hungarian_ci;
CREATE TABLE uzenetek (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(120) NOT NULL, email VARCHAR(120) NOT NULL, subject VARCHAR(160) NOT NULL, message TEXT NOT NULL, sender_user VARCHAR(180) NOT NULL DEFAULT 'Vendég', created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_hungarian_ci;
CREATE TABLE kepek (id INT AUTO_INCREMENT PRIMARY KEY, title VARCHAR(160) NOT NULL, filename VARCHAR(255) NOT NULL, uploaded_by VARCHAR(80) NOT NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_hungarian_ci;
...
INSERT INTO users (csaladi_nev, utonev, login, password_hash) VALUES ('Admin', 'Felhasználó', 'admin', '$2y$12$CU1FgP72araq4IjRebqNFu.1fgTp.O07plMBuVAW.eXmsDXnAiDI2');

INSERT INTO kepek (title, filename, uploaded_by, created_at) VALUES ('Homokbuckás útvonal', 'assets/img/galeria-homokbucka.svg', 'admin', '2026-05-18 09:00:00');
INSERT INTO kepek (title, filename, uploaded_by, created_at) VALUES ('Pallósor a lápvidéken', 'assets/img/galeria-pallo.svg', 'admin', '2026-05-18 09:05:00');
INSERT INTO kepek (title, filename, uploaded_by, created_at) VALUES ('Tanösvény tábla', 'assets/img/galeria-tabla.svg', 'admin', '2026-05-18 09:10:00');
